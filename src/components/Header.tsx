...
const Header = () => {
    ...
    // Chỉ trang chủ có header trong suốt khi chưa cuộn, các trang khác có màu nền ngay từ đầu
                ...
                <div className="max-w-[1200px] mx-auto flex h-full justify-between items-center px-2">
                    ...
                    <div className="hidden md:flex items-center gap-5">
                        <nav className="flex items-center gap-7">
                            <Link to="/" className="relative group transition-colors py-2 hover:text-primary font-lobster text-2xl" style={{ fontFamily: "'Lobster', cursive" }}>
                            ...
                            <Link to="/menu" className="relative group transition-colors py-2 hover:text-primary font-lobster text-2xl" style={{ fontFamily: "'Lobster', cursive" }}>
                                <span className="relative z-10" style={{ fontFamily: "'Lobster', cursive" }}>Thực đơn</span>
                                <span className="absolute left-0 -bottom-0.5 h-0.5 w-0 bg-primary transition-all duration-300 group-hover:w-full"></span>
                            </Link>
                            <Link to="/order-tracking" className="relative group transition-colors py-2 hover:text-primary font-lobster text-2xl" style={{ fontFamily: "'Lobster', cursive" }}>
                                <span className="relative z-10" style={{ fontFamily: "'Lobster', cursive" }}>Đơn hàng</span>
                                <span className="absolute left-0 -bottom-0.5 h-0.5 w-0 bg-primary transition-all duration-300 group-hover:w-full"></span>
                            </Link>
                            <Link to="/contact" className="relative group transition-colors py-2 hover:text-primary font-lobster text-2xl" style={{ fontFamily: "'Lobster', cursive" }}>
                    ...
                    <div className="flex items-center gap-3 px-1">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className={`cursor-pointer relative hover:text-primary transition-colors p-1 hidden md:inline-flex ${isCartAnimating ? 'animate-cart-bounce' : ''}`}
                        ...
                        <div className="relative hidden md:block">
                            {user ? (
                                <button onClick={handleUserMenuToggle} className="cursor-pointer hover:text-primary transition-colors p-1">
                                    <UserIcon size={24} />
                                </button>
                            ) : (
                                <Link to="/login" className="hover:text-primary transition-colors p-1">
                                    <UserIcon size={24} />
                                </Link>
                            )}
                            ...
                            {user && isUserMenuOpen && (
                                <div className="cursor-pointer absolute top-full right-0 mt-2 w-64 bg-white rounded-lg shadow-xl border z-50 text-secondary" onClick={() => setIsUserMenuOpen(false)}>
                                    ...
                                    <nav className="p-2">
                                        <Link to="/account" className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-gray-100 font-lobster"><Settings size={16} /> Thông tin tài khoản</Link>
                                        {user.role === 'admin' && (
                                            <Link to="/admin" className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-gray-100 font-lobster"><Shield size={16} /> Quản trị Website</Link>
                                        )}
                        ...
                        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="cursor-pointer md:hidden hover:text-primary transition-colors relative z-[110] p-2">
            ...
            <div className={`fixed top-0 left-0 h-full w-80 bg-white shadow-2xl z-[100] flex flex-col transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <nav className="flex-grow p-4 overflow-y-auto">
                    <ul className="flex flex-col gap-2">
                        {navLinks.map(link => (
                            <li key={link.href}><Link to={link.href} onClick={() => setIsMenuOpen(false)} className="block text-xl text-black font-lobster hover:text-primary py-3 px-2 rounded-md transition-colors">{link.label}</Link></li>
                        ))}
                        {user ? (
                            <>
                                {user.role === 'admin' && (
                                    <li><Link to="/admin" onClick={() => setIsMenuOpen(false)} className="block text-xl text-black font-lobster hover:text-primary py-3 px-2 rounded-md transition-colors">Admin</Link></li>
                                )}
                            ...
                            <li><Link to="/login" onClick={() => setIsMenuOpen(false)} className="block text-xl text-black font-lobster hover:text-primary py-3 px-2 rounded-md transition-colors">Đăng nhập</Link></li>
                        )}
                    </ul>
                </nav>
                    ...
                    {user ? (
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-3">
                                <Link to="/account" onClick={() => setIsMenuOpen(false)} className="flex items-center justify-center gap-2 w-full bg-blue-600 text-white font-lobster font-bold py-3 rounded-lg hover:bg-blue-700 transition-colors">
                        ...
                        <Link to="/login" onClick={() => setIsMenuOpen(false)} className="flex items-center justify-center w-full bg-primary text-light font-lobster font-bold py-3 rounded-lg hover:bg-orange-600 transition-colors">
