...
export default function MenuFilters({ onFiltersChange, className = '' }: MenuFiltersProps) {
        ...
        <div className={`relative ${className}`}>
            ...
            {isOpen && (
                <>
                    {/* Backdrop */}
                    <div
                        className="cursor-pointer fixed inset-0 z-50 pointer-events-auto"
                    ...
                    <div className="absolute top-full right-0 mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-4 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-4">
                            ...
                            {hasActiveFilters && (
                                <button
                                    ...
                                    className="cursor-pointer text-sm text-primary hover:text-orange-600 transition-colors"
                                >
                                    Xóa
                                </button>
                            )}
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-200">
                            <button
                                onClick={() => setIsOpen(false)}
                                className="cursor-pointer w-full px-4 py-2 bg-primary text-white rounded-lg hover:bg-orange-600 transition-colors"
                            >
                                Áp dụng
                            </button>
                        </div>
