...
import {
    ...
    PictureInPicture, Settings, ChevronDown, List, LayoutGrid
} from 'lucide-react';
...
const NavItem = ({ item }: { item: any }) => {
    ...
    const isActive = item.href === pathname || item.subItems?.some((sub: any) => pathname.startsWith(sub.href));
            ...
            {hasSubItems && isOpen && (
                <ul className="pl-8 py-1 space-y-1">
                    {item.subItems.map((subItem: any) => (
                        <li key={subItem.href}>
                            <Link to={subItem.href} className={`flex items-center gap-3 text-sm p-2 rounded-md ${pathname === subItem.href ? 'text-white font-semibold' : 'text-gray-400 hover:text-white'}`}>
                                {subItem.icon}
...
export default function Sidebar() {
    return (
        <aside className="w-64 bg-gray-800 text-white flex-col hidden md:flex flex-shrink-0 h-screen sticky top-0">
            <div className="h-16 flex items-center justify-center border-b border-gray-700">
                <h1 className="text-2xl font-bold tracking-widest text-white">ADMIN</h1>
            </div>
            <nav className="flex-grow pt-2 overflow-y-auto custom-scrollbar">
